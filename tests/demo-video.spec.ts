...
test.describe("Demo Video Recording", () => {
  ...
  test("menu exposes the demo video action in dev mode", async ({ page }) => {
    await page.goto("/");
    await page.waitForTimeout(500);

    // The menu button should be visible (dev mode = localhost)
    const menuBtn = page.locator('button[title="Menu"]');
    await expect(menuBtn).toBeVisible({ timeout: 5000 });

    await menuBtn.click();
    await expect(page.getByRole("button", { name: "Record demo video" })).toBeVisible({ timeout: 5000 });
  });

  test("menu shows screenshot and demo video actions", async ({ page }) => {
    await page.goto("/");
    await page.waitForTimeout(500);
    ...
    // Both actions should exist in the menu
    const screenshotBtn = page.getByRole("button", { name: "Screenshot" });
    const videoBtn = page.getByRole("button", { name: "Record demo video" });
    await expect(screenshotBtn).toBeVisible({ timeout: 5000 });
    await expect(videoBtn).toBeVisible({ timeout: 5000 });

    // Video action should appear after screenshot action
    ...
    expect(videoBox!.y).toBeGreaterThan(screenshotBox!.y);
  });
  ...
  test("recording starts with the intro template for 5 seconds before gameplay begins", async ({ page }) => {
    ...
    await page.getByRole("button", { name: "Record demo video" }).click();
