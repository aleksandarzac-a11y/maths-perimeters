...
export default function NumericKeypad({
  ...
  function press(key: string) {
    ...
    if (key === "SP") { if (value && !value.endsWith(" ")) onChange(`${value} `); return; }
    if (key === "/") { if (value && !value.includes("/")) onChange(`${value}/`); return; }
  ...
  const rows = [["7", "8", "9", "⌫"], ["4", "5", "6", "C"], ["1", "2", "3", "/"]];
    ...
    >
      ...
      >
            ...
            {row.map((btn) => (
              ...
              >
                {btn === "⌫" ? <span className={`${isMobileLandscape ? "text-[2.475rem]" : "text-[2.8rem] md:text-[2.4rem]"} leading-none`}>⌫</span>
                  : btn === "/" ? <span className={`${isMobileLandscape ? "text-[2.1rem]" : "text-[2.2rem] md:text-[1.9rem]"} leading-none`}>/</span>
                  : btn}
              </button>
            ))}
          </div>
        ...
        <div className="flex gap-0.5 mt-0.5">
          <button type="button" onClick={() => press("0")} data-autopilot-key="0" className={`${digit} flex-[1]`} style={activeKey === "0" ? pressedKeyStyle : undefined}>0</button>
          <button type="button" onClick={() => press(".")} data-autopilot-key="." className={`${op} flex-[1]`} style={activeKey === "." ? pressedKeyStyle : undefined}>.</button>
          <button type="button" onClick={() => press("SP")} data-autopilot-key="space" className={`${op} flex-[1]`} style={activeKey === "SP" ? pressedKeyStyle : undefined}>SP</button>
          <button type="button" onClick={onSubmit} disabled={!canSubmit}
            data-autopilot-key="submit"
            className={`${base} flex-[1] arcade-button disabled:opacity-40 disabled:cursor-not-allowed`}>
