...
function fitDiagramPoints(
  ...
  const offsetX = x + (width - scaledW) / 2 - minX * scale;
  const offsetY = y + (height - scaledH) / 2 - minY * scale;
...
function drawPerimeterDiagram(
  ...
  const points = attempt.diagramPoints;
  const edges = attempt.diagramEdges;
  if (!points?.length || !edges?.length) {
    return drawRippleDiagram(doc, attempt, x, y, width, height, t);
  }

  const fitted = fitDiagramPoints(points, x, y, width, height, 5);
  ...
  for (let index = 0; index < fitted.length; index++) {
    const current = fitted[index];
    const next = fitted[(index + 1) % fitted.length];
    const edge = edges[index] ?? edges[0];
    const midX = (current.x + next.x) / 2;
    const midY = (current.y + next.y) / 2;
    const label = edge.visible
      ? `${edge.label ?? edge.length}${attempt.unit ? ` ${attempt.unit}` : ""}`
      : "?";

    doc.setFillColor(edge.visible ? "#ffffff" : "#e0f2fe");
    doc.setDrawColor(edge.visible ? "#94a3b8" : "#60a5fa");
    doc.roundedRect(midX - 9, midY - 3.5, 18, 7, 1.5, 1.5, "FD");
    doc.setFontSize(5.8);
  ...
  for (let index = 0; index < fitted.length; index++) {
    ...
    doc.circle(point.x, point.y, 2.1, "FD");
    doc.setFontSize(5.5);
    doc.setFont(PDF_FONT_FAMILY, "bold");
    doc.setTextColor("#0f172a");
    doc.text(id, point.x, point.y - 4, { align: "center" });
  }
  ...
  doc.text(
    attempt.shapeType || "Perimeter shape",
    x + width / 2,
    y + height - 2,
    { align: "center" },
  );
...
export async function generateSessionPdf(summary: SessionSummary, t: TFunction, locale = "en"): Promise<Blob> {
  ...
  for (const attempt of summary.attempts) {
    ...
    // Draw the perimeter diagram used in the game
    drawPerimeterDiagram(doc, attempt, diagramX, curY + bodyPad, diagramW, diagramH, t);
