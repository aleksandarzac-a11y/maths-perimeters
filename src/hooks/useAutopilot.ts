...
export function useAutopilot({
  ...
  function scheduleAnswer(correctAnswer: number, answerText?: string) {
    const isWrong = !answerText && Math.random() < WRONG_ANSWER_RATE;
    const answer = isWrong ? String(wrongAnswer(correctAnswer)) : (answerText ?? String(correctAnswer));
    if (/[^0-9.]/.test(answer)) {
      after(rand(T.READ_DELAY), () => {
        callbacksRef.current?.setCalcValue(answer);
      });
      after(rand(T.READ_DELAY) + rand(T.PRE_SUBMIT), () => {
        const rect = getKeyRect("submit");
        if (rect) clickAt(rect.left + rect.width / 2, rect.top + rect.height / 2);
        window.setTimeout(() => {
          if (!isActiveRef.current) return;
          callbacksRef.current?.submitAnswer(answer);
          setPhantomPos(null);
  ...
  useEffect(() => {
    ...
    const { phase, targetTaps, traceTarget, answerText, tapCount } = stateRef.current;

    switch (phase) {
      case "tapping":
        scheduleTaps(traceTarget ?? targetTaps, tapCount);
        break;
      case "answering":
        scheduleAnswer(targetTaps, answerText);
        break;
      case "feedback":
        break;
      case "levelComplete":
